...
NAME_COORDS = (150, 735) #time sanga change nahune variable haru capital letter ma lekhinchha
...
def read_csv():
    ...
    with open("src/student_details.csv", "r") as file:
        ...
        for row in reader:
            data.append({"name": row[0].strip(), "code": row[1]})#strip le agadi pachai ko white space hataidincha
